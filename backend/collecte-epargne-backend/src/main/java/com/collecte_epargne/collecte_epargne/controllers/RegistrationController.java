...
public class RegistrationController {
    ...
    public ResponseEntity<?> registerClient(@Valid @RequestBody ClientRegistrationRequest request) {
        try {
            ...
            while (clientRepository.findById(codeClient).isPresent()) {
                codeClient = "CLI" + System.currentTimeMillis();
            }

            // Générer un numéro client unique
            String numeroClient = "NUM" + UUID.randomUUID().toString().substring(0, 8).toUpperCase();
            while (clientRepository.findByNumeroClient(numeroClient).isPresent()) {
                numeroClient = "NUM" + UUID.randomUUID().toString().substring(0, 8).toUpperCase();
            }
