...
class RoleOption {
  ...
  RoleOption({required this.id, required this.code, required this.nom, this.description});
...
class AuthApi {
  // ⚠️ IMPORTANT: Le port 59812 est le port du serveur Flutter web (frontend).
  // Les appels API doivent aller vers le backend Spring qui tourne sur un port différent (8082 par défaut).
  static String _webBase() {
    // Port backend Spring selon application.properties : 8082
    // Si ton backend tourne sur un autre port, change cette valeur
    return 'http://localhost:8082';
  }

  static const _androidBase = 'http://10.0.2.2:8082'; // Android emulator loopback vers localhost:8082
  static const _defaultBase = 'http://localhost:8082';
  ...
  static Future<List<RoleOption>> fetchRoles() async {
    ...
    try {
      final res = await http.get(uri).timeout(
        ...
        onTimeout: () => throw Exception('Timeout: le serveur ne répond pas sur $uri'),
      );
      if (res.statusCode != 200) {
        final preview = res.body.length > 120 ? '${res.body.substring(0, 120)}…' : res.body;
        throw Exception('Impossible de charger les rôles (${res.statusCode}) : $preview');
      }
      ...
      try {
        ...
        if (decoded is! List) {
          throw Exception('Réponse inattendue /api/roles (attendu tableau JSON)');
        }
        return decoded.map((e) => RoleOption.fromJson(e as Map<String, dynamic>)).toList();
      } catch (e) {
      ...
      throw Exception('Erreur réseau: ${e.message}. Vérifiez que le backend est lancé sur ${_baseUrl()}');
    } catch (e) {
  ...
  static Future<UserModel> login({
    ...
    try {
      ...
      final res = await http.post(
        ...
        const Duration(seconds: 10),
        onTimeout: () {
          throw Exception('Timeout: Le backend ne répond pas sur ${getBaseUrl()}. Vérifiez qu\'il est démarré.');
        },
      );
      ...
      if (res.statusCode == 200) {
        ...
        print('✅ Connexion réussie: ${data['login']} (${data['nomRole']})');
        return UserModel.fromJson(data);
      } else {
        final error = jsonDecode(res.body);
        final errorMsg = error['error'] ?? 'Erreur de connexion';
        print('❌ Erreur de connexion: $errorMsg');
        throw Exception(errorMsg);
      }
    } on http.ClientException catch (e) {
      final baseUrl = getBaseUrl();
      final errorMsg = 'Le backend n\'est pas accessible sur $baseUrl.\n'
          'Vérifiez que:\n'
  ...
  static Future<void> registerClient({
    ...
    String? address,
    String? collectorMatricule, // Matricule du collecteur parrain (optionnel, "0000" si pas de parrain)
  }) async {
    final payload = {
      'fullName': fullName,
      'phone': phone,
      ...
      'collectorMatricule': collectorMatricule ?? '0000', // Par défaut "0000" si non fourni
